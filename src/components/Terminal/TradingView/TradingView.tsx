import React, { useState } from 'react';
import { Bitcoin } from 'lucide-react';
import OrderForm from './OrderForm';
import PriceChart from './PriceChart';
import OrderBook from './OrderBook';
import { useTradingData } from '../../../hooks/useTradingData';

export default function TradingView() {
  const [selectedTab, setSelectedTab] = useState<'buy' | 'sell'>('buy');
  const { priceData, orderBook } = useTradingData();

  const handleOrder = (price: string, amount: string) => {
    console.log('Order submitted:', { type: selectedTab, price, amount });
  };

  return (
    <div className="bg-surface rounded-xl border border-accent/20 overflow-hidden">
      <div className="p-4 border-b border-accent/20">
        <div className="flex items-center gap-2">
          <Bitcoin className="w-6 h-6 text-accent" />
          <h2 className="text-sm text-primary">CHUCKIE</h2>
          <span className="text-xs text-primary/60">Floor: 0.89 BTC</span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-accent/20">
        <div className="lg:col-span-2">
          <PriceChart data={priceData} />
        </div>
        <div className="p-4 space-y-4">
          <div className="flex rounded-lg overflow-hidden border border-accent/20">
            <button
              onClick={() => setSelectedTab('buy')}
              className={`flex-1 px-3 py-1.5 text-xs ${
                selectedTab === 'buy'
                  ? 'bg-accent/10 text-accent'
                  : 'text-primary hover:text-accent'
              }`}
            >
              Buy
            </button>
            <button
              onClick={() => setSelectedTab('sell')}
              className={`flex-1 px-3 py-1.5 text-xs ${
                selectedTab === 'sell'
                  ? 'bg-accent/10 text-accent'
                  : 'text-primary hover:text-accent'
              }`}
            >
              Sell
            </button>
          </div>
          <OrderForm type={selectedTab} onSubmit={handleOrder} />
        </div>
      </div>

      <div className="p-4 border-t border-accent/20">
        <OrderBook asks={orderBook.asks} bids={orderBook.bids} />
      </div>
    </div>
  );
}